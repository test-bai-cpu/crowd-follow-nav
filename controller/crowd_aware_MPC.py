...
class CrowdAwareMPC:
    ...
    def init_solver(self, ipopt_print_level=0):
        ...
        for t in range(self.mpc_horizon):
            ...
            for human_i in range(self.max_humans):
                ...
                dist_to_robot = cs.sqrt((x_var[0,t] - human_x)**2 + (x_var[1,t] - human_y)**2)
                valid_human = (dist_to_robot < self.max_obs_distance)
                dist = cs.if_else(valid_human, dist_to_robot, 1e6)
            ...
            cost_safe += cs.fmax(0, (self.d_safe - min_distance)) * self.w_safe
        ...
        position_cost = cs.norm_2(x_var[:2, self.mpc_horizon-1] - follow_pos)
        weight = cs.fmax(0, (cs.norm_2(x_var[:2, self.mpc_horizon-1] - follow_pos) - 1) / 100)
        weight = cs.fmin(1, weight)
        velocity_cost = weight * cs.sqrt(ad**2 + ld**2)
        
        cost_follow = self.w_follow * (position_cost + velocity_cost)
        ...
        # cost = cost_follow * if_follow + cost_smooth + cost_safe
        # cost = cost_follow * if_follow + cost_safe + cost_smooth
        cost = cost_follow * if_follow + cost_safe
    ...
    def get_action(self, current_state, target, nearby_human_state, follow_state):
        ...
        try:
            ...
            # print("Fx_var (state trajectory):", x_var)
            # print("Fu_var (control inputs):", u_var)

            # follow_vel = opti.debug.value(self.opti_dict["follow_vel"])
            # follow_pos = opti.debug.value(self.opti_dict["follow_pos"])

            # print("Ffollow_vel: ", follow_vel)
            # print("Ffollow_pos: ", follow_pos)
            ...
            # print("-----Fcheck safety cost-----")
            # cost_safe = self.get_cost_safe(x_var, nearby_human_pos)
            # print("Fsafety cost: ", cost_safe)
            # print("---------------------------")
            # print("-----Fcheck smooth cost-----")
            # cost_smooth = self.get_cost_smooth(u_var)
            # print("Fsmooth cost: ", cost_smooth)
            # print("---------------------------")
            # print("-----Fcheck follow cost-----")
            # cost_follow = self.get_cost_follow(x_var, u_var, follow_pos, follow_vel, if_follow)
            # print("Fcost_follow: ", cost_follow)
            # print("---------------------------")
            # print("Ftotal cost: ", cost_safe + cost_smooth + cost_follow)
        ...
        # print("-----check safety cost-----")
        # cost_safe = self.get_cost_safe(x_var, nearby_human_pos)
        # print("safety cost: ", cost_safe)
        # print("---------------------------")
        # print("-----check smooth cost-----")
        # cost_smooth = self.get_cost_smooth(u_var)
        # print("smooth cost: ", cost_smooth)
        # print("---------------------------")
        # print("-----check follow cost-----")
        # cost_follow = self.get_cost_follow(x_var, u_var, follow_pos, follow_vel, if_follow)
    ...
    def get_cost_safe(self, x_var, nearby_human_pos):
        ...
        for t in range(self.mpc_horizon):
            ...
            # Compute safety cost (only when close to humans)
            cost_safe_t = cs.fmax(0, (self.d_safe - min_distance)) * self.w_safe
            print(f"cost_safe_{t}: ", cost_safe_t)
            cost_safe += cost_safe_t
    ...
    def get_cost_follow(self, x_var, u_var, follow_pos, follow_vel, if_follow):
        ...
        weight = cs.fmax(0, cs.norm_2(x_var[:2, self.mpc_horizon-1] - follow_pos) / 10 - 0.1)
        print("weight: ", weight)
        velocity_cost = weight * cs.sqrt(ad**2 + ld**2)
